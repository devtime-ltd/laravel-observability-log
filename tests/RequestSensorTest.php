...
describe('config options', function () {
    ...
    it('falls back to $request->ip() when resolve_ip returns an empty string', function () {
        config([
            'observability-log.requests.channel' => 'test-channel',
            'observability-log.requests.resolve_ip' => fn () => '',
        ]);
    ...
    it('fails closed when obfuscate_ip throws (logs null IP, not the raw value)', function () {
        config([
            'observability-log.requests.channel' => 'test-channel',
            'observability-log.requests.obfuscate_ip' => function () {
                throw new RuntimeException('obfuscator broke');
            },
        ]);
        ...
        Log::shouldReceive('channel')->with('test-channel')->andReturn($channel);
        Log::shouldReceive('error')->once()->with(Mockery::pattern('/obfuscate_ip callable threw.*obfuscator broke/'));
